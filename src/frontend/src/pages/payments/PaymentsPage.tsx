...
import { canApproveBilling, canDelete } from "../../lib/roleAccess";
...
export default function PaymentsPage() {
  ...
  const canUserCreatePayment =
    currentUser && canApproveBilling(currentUser.role);
        ...
        <CardContent>
          {sortedPayments.length === 0 ? (
            <div className="text-center py-8 text-slate-500">
              No payments found.{" "}
              {canUserCreatePayment && 'Click "Record Payment" to create one.'}
            </div>
          ) : (
            <Table>
              <TableHeader>
