...
export function useLogin() {
  ...
  return useMutation({
    ...
    onSuccess: (profile) => {
      ...
      // Also invalidate users list so it refetches with newly-granted permissions
...
// -----------------------------------------------------------------------
export function useGetCallerUserProfile(authRequired = true) {
  ...
  // Latch isFetched — once true, never go back to false even if actor re-fetches
  const fetchedRef = useRef(false);
  if (authRequired && !!actor && query.isFetched) fetchedRef.current = true;
  const isFetched = authRequired ? fetchedRef.current : true;

  // Only count as loading if we haven't fetched yet
  const isLoading = authRequired
    ? !fetchedRef.current && (actorFetching || query.isLoading)
    : false;
...
export function useApproveBillPM() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (!actor) throw new Error("Actor not available");
      return actor.updateBillPMApproval(
        data.billId,
        data.pmApproved,
        data.pmDebit,
        data.pmNote,
...
export function useApproveBillQC() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (!actor) throw new Error("Actor not available");
      return actor.updateBillQCApproval(
        data.billId,
        data.qcApproved,
        data.qcDebit,
        data.qcNote,
...
export function useApproveBillBilling() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (!actor) throw new Error("Actor not available");
      return actor.updateBillBillingApproval(
        data.billId,
        data.billingApproved,
        data.finalAmount,
        data.status,
      );
    },
...
export function useApproveNMRPM() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (!actor) throw new Error("Actor not available");
      return actor.updateNMRPMApproval(
        data.nmrId,
        data.pmApproved,
        data.pmDebit,
        data.pmNote,
...
export function useApproveNMRQC() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (!actor) throw new Error("Actor not available");
      return actor.updateNMRQCApproval(
        data.nmrId,
        data.qcApproved,
        data.qcDebit,
        data.qcNote,
...
export function useApproveNMRBilling() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (!actor) throw new Error("Actor not available");
      return actor.updateNMRBillingApproval(
        data.nmrId,
        data.billingApproved,
        data.finalAmount,
        data.status,
